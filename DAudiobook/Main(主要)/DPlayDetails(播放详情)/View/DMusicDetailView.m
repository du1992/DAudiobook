...
- (void)setLayout{
    ...
    [self.songLB mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(kScreenWidth-160, 20));
     }];
...
-(void)interfaceRefresh{
   ...
   [self.songIcon sd_setImageWithURL:[NSURL URLWithString:songModel.coverimg] placeholderImage:ImageNamed(@"timeline_image_placeholder")];
    ...
    if (!time) {
        time = [NSString stringWithFormat:@"%@",DUserDefaultsGET(TOTALTIME)];
    }
...
- (UISlider *)sliderProgress{
    if (!_sliderProgress) {
        _sliderProgress = [[UISlider alloc] init];
         _sliderProgress.minimumTrackTintColor =AppColor(255, 209, 2);
...
- (UIButton *)singlecycleBtn{
    if (!_singlecycleBtn) {
        ...
        [_singlecycleBtn setImage:ImageNamed(@"singlecycle1") forState:UIControlStateSelected];
...
- (UIButton *)previousBtn{
    if (!_previousBtn) {
        ...
        [_previousBtn setImage:ImageNamed(@"icons_previous_music1") forState:UIControlStateNormal];
...
- (UIButton *)playAndPauseBtn{
    if (!_playAndPauseBtn) {
        ...
        [_playAndPauseBtn setImage:ImageNamed(@"icons_play_music1") forState:UIControlStateNormal];
        [_playAndPauseBtn setImage:ImageNamed(@"icons_stop_music1") forState:UIControlStateSelected];
