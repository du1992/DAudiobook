...
@property (nonatomic, strong) DSlider *slider;
...
- (UIView *)bottomBar {
    if (!_bottomBar) {
        ...
        DSlider *slider = [[DSlider alloc] init];
        slider.value = 0.0f;
        ...
        slider.valueChangeBlock = ^(DSlider *slider){
            [weakSelf sliderValueChange:slider];
        };
        slider.finishChangeBlock = ^(DSlider *slider){
            [weakSelf finishChange];
        };
        slider.draggingSliderBlock = ^(DSlider *slider){
            [weakSelf dragSlider];
        };
...
- (void)smallScreen {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if (self->_isDown) {
            self.frame = CGRectMake(self.playerOriginalFrame.origin.x, self.playerOriginalFrame.origin.y+40, self.playerOriginalFrame.size.width, self.playerOriginalFrame.size.height);
        }else{
...
- (void)sliderValueChange:(DSlider *)slider {
